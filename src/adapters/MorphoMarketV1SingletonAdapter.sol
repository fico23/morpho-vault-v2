...
contract MorphoMarketV1SingletonAdapter is IMorphoMarketV1Adapter {
    ...
    function allocate(bytes memory data, uint256 assets, bytes4, address) external returns (bytes32[] memory, int256) {
        ...
        MarketParams[] memory marketsParams = abi.decode(data, (MarketParams[]));
        MarketParams memory marketParams = marketsParams[0];
        require(marketParams.loanToken == asset, LoanAssetMismatch());

        uint256 oldAllocation = MorphoBalancesLib.expectedSupplyAssets(IMorpho(morpho), marketParams, address(this));
        ...
        returns (bytes32[] memory, int256)
    {
        ...
        for (uint256 i; i < marketsParams.length && assets != 0; ++i) {
            marketParams = marketsParams[i];
            require(marketParams.loanToken == asset, LoanAssetMismatch());

            uint256 ownedAssets = MorphoBalancesLib.expectedSupplyAssets(IMorpho(morpho), marketParams, address(this));
            uint256 assetsToWithdraw = min(ownedAssets, assets);
            IMorpho(morpho).withdraw(marketParams, assets, 0, address(this), address(this));
            assets -= assetsToWithdraw;
            uint256 newAllocation = MorphoBalancesLib.expectedSupplyAssets(IMorpho(morpho), marketParams, address(this));
            updateList(marketParams, ownedAssets, newAllocation);

            change += int256(newAllocation) - int256(ownedAssets);
        }
    ...
    function allocation(MarketParams memory marketParams) public view returns (uint256) {
        return MorphoBalancesLib.expectedSupplyAssets(IMorpho(morpho), marketParams, address(this));
    }

    /// @dev Returns adapter's ids.
    function ids(MarketParams memory) public view returns (bytes32[] memory) {
